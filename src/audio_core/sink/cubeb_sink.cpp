...
CubebSink::CubebSink(std::string_view target_device_name) {
    ...
    if (cubeb_init(&ctx, "sudachi", nullptr) != CUBEB_OK) {
...
std::vector<std::string> ListCubebSinkDevices(bool capture) {
    ...
    if (cubeb_init(&ctx, "sudachi Device Enumerator", nullptr) != CUBEB_OK) {
        LOG_CRITICAL(Audio_Sink, "cubeb_init failed");
        return {};
    }
...
bool IsCubebSuitable() {
    ...
    // Init cubeb
    if (cubeb_init(&ctx, "sudachi Latency Getter", nullptr) != CUBEB_OK) {
