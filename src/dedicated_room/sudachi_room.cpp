...
static void PrintVersion() {
    LOG_INFO(Network, "sudachi dedicated room {} {} Libnetwork: {}", Common::g_scm_branch,
             Common::g_scm_desc, Network::network_version);
}

/// The magic text at the beginning of a sudachi-room ban list file.
static constexpr char BanListMagic[] = "SudachiRoom-BanList-1";
...
int main(int argc, char** argv) {
    ...
    std::string log_file = "sudachi-room.log";
    std::string bind_address;
    u64 preferred_game_id = 0;
    u32 port = Network::DefaultRoomPort;
    u32 max_members = 16;
    bool enable_sudachi_mods = false;
    ...
    static struct option long_options[] = {
        ...
        {"enable-sudachi-mods", no_argument, 0, 'e'},
    ...
    if (announce) {
        if (username.empty()) {
            ...
            PadToken(token);
            Settings::values.sudachi_username = UsernameFromDisplayToken(token);
            username = Settings::values.sudachi_username.GetValue();
            Settings::values.sudachi_token = TokenFromDisplayToken(token);
        } else {
            LOG_INFO(Network, "Hosting a public room");
            Settings::values.web_api_url = web_api_url;
            Settings::values.sudachi_username = username;
            Settings::values.sudachi_token = token;
        }
    }
    if (!announce && enable_sudachi_mods) {
        enable_sudachi_mods = false;
        LOG_INFO(Network, "Can not enable sudachi Moderators for private rooms");
    }
...
#else
        LOG_INFO(Network,
                 "sudachi Web Services is not available with this build: validation is disabled.");
    ...
    if (auto room = network.GetRoom().lock()) {
        ...
        if (!room->Create(room_name, room_description, bind_address, static_cast<u16>(port),
                          ...
                          std::move(verify_backend), ban_list, enable_sudachi_mods)) {
