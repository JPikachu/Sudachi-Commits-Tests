...
NvResult nvhost_as_gpu::AllocateSpace(IoctlAllocSpace& params) {
    ...
    if (params.page_size != VM::SUDACHI_PAGESIZE && params.page_size != vm.big_page_size) {
        return NvResult::BadValue;
    }
    ...
    allocation_map[params.offset] = {
        ...
        .big_pages = params.page_size != VM::SUDACHI_PAGESIZE,
    };
...
void nvhost_as_gpu::FreeMappingLocked(u64 offset) {
    ...
    if (!mapping->fixed) {
        ...
        u32 page_size{mapping->big_page ? vm.big_page_size : VM::SUDACHI_PAGESIZE};
...
NvResult nvhost_as_gpu::MapBufferEx(IoctlMapBufferEx& params) {
    ...
    bool big_page{[&]() {
        if (Common::IsAligned(handle->align, vm.big_page_size)) {
            return true;
        } else if (Common::IsAligned(handle->align, VM::SUDACHI_PAGESIZE)) {
            return false;
        } else {
            ASSERT(false);
            return false;
    ...
    if ((params.flags & MappingFlags::Fixed) != MappingFlags::None) {
        ...
        u32 page_size{big_page ? vm.big_page_size : VM::SUDACHI_PAGESIZE};
