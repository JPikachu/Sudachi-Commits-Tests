...
std::optional<Key128> DeriveSDSeed() {
    ...
    const auto sd_private_path =
        Common::FS::GetSudachiPath(Common::FS::SudachiPath::SDMCDir) / "Nintendo/Contents/private";
...
void KeyManager::ReloadKeys() {
    ...
    if (!Common::FS::CreateDir(sudachi_keys_dir)) {
        LOG_ERROR(Core, "Failed to create the keys directory.");
    }

    if (!Common::FS::Exists(sudachi_keys_dir / "prod.keys") && !Common::FS::Exists(sudachi_keys_dir / "title.keys")) {
        LOG_ERROR(Core, "Unable to find prod and title keys files");
        return;
    }
    ...
    if (Settings::values.use_dev_keys) {
        ...
        LoadFromFile(sudachi_keys_dir / "dev.keys", false);
    } else {
        dev_mode = false;
        LoadFromFile(sudachi_keys_dir / "prod.keys_autogenerated", false);
        LoadFromFile(sudachi_keys_dir / "prod.keys", false);
    }

    LoadFromFile(sudachi_keys_dir / "title.keys_autogenerated", true);
    LoadFromFile(sudachi_keys_dir / "title.keys", true);
    LoadFromFile(sudachi_keys_dir / "console.keys_autogenerated", false);
    LoadFromFile(sudachi_keys_dir / "console.keys", false);
}
...
void KeyManager::WriteKeyToFile(KeyCategory category, std::string_view keyname,
    ...
    const auto path = sudachi_keys_dir / filename;
...
bool KeyManager::KeyFileExists(bool title) {
    ...
    if (title) {
        return Common::FS::Exists(sudachi_keys_dir / "title.keys");
    }

    if (Settings::values.use_dev_keys) {
        return Common::FS::Exists(sudachi_keys_dir / "dev.keys");
    }

    return Common::FS::Exists(sudachi_keys_dir / "prod.keys");
}
