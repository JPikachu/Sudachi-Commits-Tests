...
std::optional<NcaID> RegisteredCache::GetNcaIDFromMetadata(u64 title_id,
    ...
    const auto res1 = CheckMapForContentRecord(sudachi_meta, title_id, type);
    if (res1)
        return res1;
...
void RegisteredCache::AccumulateSudachiMeta() {
    const auto meta_dir = dir->GetSubdirectory("sudachi_meta");
    if (meta_dir == nullptr) {
        return;
    }
    ...
    for (const auto& file : meta_dir->GetFiles()) {
        ...
        CNMT cnmt(file);
        sudachi_meta.insert_or_assign(cnmt.GetTitleID(), std::move(cnmt));
    }
}
...
std::optional<u32> RegisteredCache::GetEntryVersion(u64 title_id) const {
    ...
    const auto sudachi_meta_iter = sudachi_meta.find(title_id);
    if (sudachi_meta_iter != sudachi_meta.cend()) {
        return sudachi_meta_iter->second.GetTitleVersion();
    }
...
void RegisteredCache::IterateAllMetadata(
    ...
    for (const auto& kv : sudachi_meta) {
        const auto& cnmt = kv.second;
        for (const auto& rec : cnmt.GetContentRecords()) {
            if (GetFileAtID(rec.nca_id) != nullptr && filter(cnmt, rec)) {
                out.push_back(proc(cnmt, rec));
...
InstallResult RegisteredCache::InstallEntry(const NCA& nca, TitleType type,
    ...
    if (!RawInstallSudachiMeta(new_cnmt)) {
        return InstallResult::ErrorMetaFailed;
    }
...
bool RegisteredCache::RemoveExistingEntry(u64 title_id) const {
    ...
    // If patch entries for any program exist in sudachi meta, remove them
    for (u8 i = 0; i < 0x10; i++) {
        const auto meta_dir = dir->CreateDirectoryRelative("sudachi_meta");
...
bool RegisteredCache::RawInstallSudachiMeta(const CNMT& cnmt) {
    ...
    Refresh();
    return std::find_if(sudachi_meta.begin(), sudachi_meta.end(),
