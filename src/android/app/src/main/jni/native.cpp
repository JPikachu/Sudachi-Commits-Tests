...
int Java_org_sudachi_sudachi_1emu_NativeLibrary_installFileToNand(JNIEnv* env, jobject instance,
    ...
    const auto callback = [env, jcallback, jlambdaInvokeMethod](size_t max, size_t progress) {
        ...
        jobject jMax = env->NewObject(longClass, longConstructor, static_cast<jlong>(max));
        jobject jProgress = env->NewObject(longClass, longConstructor, static_cast<jlong>(progress));
        ...
        jboolean wasCancelled = env->CallBooleanMethod(jwasCancelled, booleanValueMethod);

        env->DeleteLocalRef(jMax);
        env->DeleteLocalRef(jProgress);
        env->DeleteLocalRef(jwasCancelled);
