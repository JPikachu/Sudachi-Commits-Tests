...
object GameIconUtils {
    private val imageLoader = ImageLoader.Builder(SudachiApplication.appContext)
        .components {
            add(GameIconKeyer())
            add(GameIconFetcher.Factory())
        }
        .memoryCache {
            MemoryCache.Builder(SudachiApplication.appContext)
                .maxSizePercent(0.25)
                .build()
        }
        .build()
    ...
    fun loadGameIcon(game: Game, imageView: ImageView) {
        val request = ImageRequest.Builder(SudachiApplication.appContext)
            .data(game)
            .target(imageView)
    ...
    suspend fun getShortcutIcon(lifecycleOwner: LifecycleOwner, game: Game): IconCompat {
        val layerDrawable = ResourcesCompat.getDrawable(
            SudachiApplication.appContext.resources,
            R.drawable.shortcut,
            null
        ) as LayerDrawable
        layerDrawable.setDrawableByLayerId(
            R.id.shortcut_foreground,
            getGameIcon(lifecycleOwner, game).toDrawable(SudachiApplication.appContext.resources)
        )
        val inset = SudachiApplication.appContext.resources
