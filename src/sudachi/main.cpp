...
#include "sudachi/multiplayer/state.h"
...
#include "sudachi/configuration/configure_dialog.h"
#include "sudachi/configuration/configure_input_per_game.h"
#include "sudachi/configuration/qt_config.h"
#include "sudachi/debugger/console.h"
#include "sudachi/debugger/controller.h"
#include "sudachi/debugger/profiler.h"
#include "sudachi/debugger/wait_tree.h"
#include "sudachi/discord.h"
#include "sudachi/game_list.h"
#include "sudachi/game_list_p.h"
#include "sudachi/hotkeys.h"
#include "sudachi/install_dialog.h"
...
#include "sudachi/discord_impl.h"
#endif
...
void GMainWindow::ShowTelemetryCallout() {
    ...
    const QString telemetry_message =
        tr("<a href='https://sudachi-emu.org/help/feature/telemetry/'>Anonymous "
           "data is collected</a> to help improve sudachi. "
...
static void RemoveCachedContents() {
    const auto cache_dir = Common::FS::GetSudachiPath(Common::FS::SudachiPath::CacheDir);
...
GMainWindow::GMainWindow(std::unique_ptr<QtConfig> config_, bool has_broken_vulkan)
    ...
    LOG_INFO(Frontend, "sudachi Version: {}", sudachi_build_version);
    LogRuntimes();
#ifdef ARCHITECTURE_x86_64
    ...
    //: TRANSLATORS: This string is shown to the user to explain why sudachi needs to prevent the
...
#ifdef SUDACHI_ENABLE_COMPATIBILITY_REPORTING
...
void GMainWindow::InitializeHotkeys() {
    ...
    LinkActionShortcut(ui->action_Exit, QStringLiteral("Exit sudachi"));
...
void GMainWindow::ConnectMenuEvents() {
    ...
    // Help
    connect_menu(ui->action_Open_sudachi_Folder, &GMainWindow::OnOpenSudachiFolder);
    ...
    if (result != Core::SystemResultStatus::Success) {
        ...
        case Core::SystemResultStatus::ErrorVideoCore:
            QMessageBox::critical(
                ...
                tr("sudachi has encountered an error while running the video core. "
                   "This is usually caused by outdated GPU drivers, including integrated ones. "
                   "Please see the log for more details. "
                   "For more information on accessing the log, please see the following page: "
                   "<a href='https://sudachi-emu.org/help/reference/log-files/'>"
        ...
        default:
            if (result > Core::SystemResultStatus::ErrorLoader) {
                ...
                const auto description =
                    tr("%1<br>Please follow <a href='https://sudachi-emu.org/help/quickstart/'>the "
                       "sudachi quickstart guide</a> to redump your files.<br>You can refer "
                       "to the sudachi wiki</a> or the sudachi Discord</a> for help.",
...
void GMainWindow::BootGame(const QString& filename, Service::AM::FrontendAppletParameters params,
                           StartGameType type) {
    LOG_INFO(Frontend, "sudachi starting...");
...
void GMainWindow::OnGameListOpenFolder(u64 program_id, GameListOpenTarget target,
    ...
    case GameListOpenTarget::ModData: {
        ...
        path = Common::FS::GetSudachiPath(Common::FS::SudachiPath::LoadDir) /
...
void GMainWindow::OnTransferableShaderCacheOpenFile(u64 program_id) {
    const auto shader_cache_dir = Common::FS::GetSudachiPath(Common::FS::SudachiPath::ShaderDir);
...
void GMainWindow::OnGameListRemoveInstalledEntry(u64 program_id, InstalledEntryType type) {
    ...
    Common::FS::RemoveDirRecursively(Common::FS::GetSudachiPath(Common::FS::SudachiPath::CacheDir) /
                                     "game_list");
    game_list->PopulateAsync(UISettings::values.game_dirs);
}
...
void GMainWindow::OnGameListNavigateToGamedbEntry(u64 program_id,
    ...
    QDesktopServices::openUrl(QUrl(QStringLiteral("https://sudachi-emu.org/game/") + directory));
}
...
void GMainWindow::OnGameListCreateShortcut(u64 program_id, const std::string& game_path,
    ...
    // Get path to sudachi executable
    const QStringList args = QApplication::arguments();
    std::filesystem::path sudachi_command = args[0].toStdString();
    // If relative path, make it an absolute path
    if (sudachi_command.c_str()[0] == '.') {
        sudachi_command = Common::FS::GetCurrentDir() / sudachi_command;
    }
    // Shortcut path
    ...
    if (sudachi_command.string().ends_with(appimage_ending) &&
    ...
    if (GMainWindow::CreateShortcutLink(shortcut_path, comment, out_icon_path, sudachi_command,
...
void GMainWindow::OnMenuReportCompatibility() {
    ...
    if (!Settings::values.sudachi_token.GetValue().empty() &&
        !Settings::values.sudachi_username.GetValue().empty()) {
        CompatDB compatdb{system->TelemetrySession(), this};
        compatdb.exec();
    } else {
        QMessageBox::critical(
            this, tr("Missing sudachi Account"),
            tr("In order to submit a game compatibility test case, you must link your sudachi "
               "account.<br><br/>To link your sudachi account, go to Emulation &gt; Configuration "
               "&gt; "
               "Web."));
    }
#else
...
void GMainWindow::OnOpenModsPage() {
    OpenURL(QUrl(QStringLiteral("https://github.com/sudachi-emu/sudachi/wiki/Switch-Mods")));
}

void GMainWindow::OnOpenQuickstartGuide() {
    OpenURL(QUrl(QStringLiteral("https://sudachi-emu.org/help/quickstart/")));
}

void GMainWindow::OnOpenFAQ() {
    OpenURL(QUrl(QStringLiteral("https://sudachi-emu.org/wiki/faq/")));
}
...
void GMainWindow::OnConfigure() {
    ...
    if (result != QDialog::Accepted && !UISettings::values.configuration_applied &&
        ...
        if (!Common::FS::RemoveDirRecursively(
                Common::FS::GetSudachiPath(Common::FS::SudachiPath::CacheDir) / "game_list")) {
        ...
        if (QtProgressCallback(
            ...
            QMessageBox::warning(
                ...
                tr("Firmware installation cancelled, firmware may be in bad state, "
                   "restart sudachi or re-install firmware."));
            return;
        }
    }
...
void GMainWindow::OnInstallDecryptionKeys() {
    ...
    const auto sudachi_keys_dir = Common::FS::GetSudachiPath(Common::FS::SudachiPath::KeysDir);
    for (auto key_file : source_key_files) {
        std::filesystem::path destination_key_file = sudachi_keys_dir / key_file.filename();
...
void GMainWindow::MigrateConfigFiles() {
    const auto config_dir_fs_path = Common::FS::GetSudachiPath(Common::FS::SudachiPath::ConfigDir);
...
void GMainWindow::UpdateWindowTitle(std::string_view title_name, std::string_view title_version,
    ...
    const auto sudachi_title = fmt::format("sudachi | {}-{}", branch_name, description);
    const auto override_title =
        fmt::format(fmt::runtime(std::string(Common::g_title_bar_format_idle)), build_id);
    const auto window_title = override_title.empty() ? sudachi_title : override_title;
    ...
    if (!ContentManager::AreKeysPresent()) {
        QMessageBox::warning(
            ...
            tr("Encryption keys are missing. "
               "<br>Please follow <a href='https://sudachi-emu.org/help/quickstart/'>the sudachi "
...
bool GMainWindow::ConfirmClose() {
    ...
    const auto text = tr("Are you sure you want to close sudachi?");
    return question(this, tr("sudachi"), text);
}
...
bool GMainWindow::ConfirmForceLockedExit() {
    ...
    const auto text = tr("The currently running application has requested sudachi to not exit.\n\n"
...
int main(int argc, char* argv[]) {
    ...
    QCoreApplication::setOrganizationName(QStringLiteral("sudachi team"));
    QCoreApplication::setApplicationName(QStringLiteral("sudachi"));
    ...
    // suffix.
    QGuiApplication::setDesktopFileName(QStringLiteral("org.sudachi_emu.sudachi"));
#endif
