...
#include "sudachi/multiplayer/client_room.h"
#include "sudachi/multiplayer/lobby.h"
#include "sudachi/multiplayer/lobby_p.h"
#include "sudachi/multiplayer/message.h"
#include "sudachi/multiplayer/state.h"
#include "sudachi/multiplayer/validation.h"
...
Lobby::Lobby(QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->nickname->text().isEmpty() || ui->nickname->text() == QStringLiteral("sudachi")) {
        if (!Settings::values.sudachi_username.GetValue().empty()) {
            ui->nickname->setText(
                QString::fromStdString(Settings::values.sudachi_username.GetValue()));
        } else if (!GetProfileUsername().empty()) {
            ui->nickname->setText(QString::fromStdString(GetProfileUsername()));
        } else {
            ui->nickname->setText(QStringLiteral("sudachi"));
        }
    }
...
#ifdef ENABLE_WEB_SERVICE
        if (!Settings::values.sudachi_username.GetValue().empty() &&
            !Settings::values.sudachi_token.GetValue().empty()) {
            WebService::Client client(Settings::values.web_api_url.GetValue(),
                                      Settings::values.sudachi_username.GetValue(),
                                      Settings::values.sudachi_token.GetValue());
