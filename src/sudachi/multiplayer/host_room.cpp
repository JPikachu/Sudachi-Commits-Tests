...
#include "sudachi/multiplayer/host_room.h"
...
HostRoomWindow::HostRoomWindow(QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->username->text().isEmpty() && !Settings::values.sudachi_username.GetValue().empty()) {
        // Use sudachi Web Service user name as nickname by default
        ui->username->setText(QString::fromStdString(Settings::values.sudachi_username.GetValue()));
    }
        ...
        if (auto room = room_network.GetRoom().lock()) {
            const bool created =
                room->Create(ui->room_name->text().toStdString(),
                             ...
                             ui->max_player->value(), Settings::values.sudachi_username.GetValue(),
                ...
                if (result.result_code != WebService::WebResult::Code::Success) {
                    QMessageBox::warning(
                        ...
                        tr("Failed to announce the room to the public lobby. In order to host a "
                           "room publicly, you must have a valid sudachi account configured in "
