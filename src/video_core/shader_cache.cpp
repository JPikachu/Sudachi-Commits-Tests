...
void ShaderCache::Register(std::unique_ptr<ShaderInfo> data, VAddr addr, size_t size) {
    ...
    const u64 page_end = (addr_end + SUDACHI_PAGESIZE - 1) >> SUDACHI_PAGEBITS;
...
void ShaderCache::RemoveEntryFromInvalidationCache(const Entry* entry) {
    const u64 page_end = (entry->addr_end + SUDACHI_PAGESIZE - 1) >> SUDACHI_PAGEBITS;
    for (u64 page = entry->addr_start >> SUDACHI_PAGEBITS; page < page_end; ++page) {
