...
BUILDBIN=/sudachi/build/bin
BINFILE=sudachi-x86_64.AppImage
...
cp -P "$BUILDBIN"/sudachi $HOME/squashfs-root/usr/bin/

curl -sL https://raw.githubusercontent.com/$GITHUB_REPOSITORY/$BRANCH/dist/sudachi.svg -o ./squashfs-root/sudachi.svg
curl -sL https://raw.githubusercontent.com/$GITHUB_REPOSITORY/$BRANCH/dist/sudachi.desktop -o ./squashfs-root/sudachi.desktop
curl -sL https://github.com/AppImage/AppImageKit/releases/download/continuous/runtime-x86_64 -o ./squashfs-root/runtime
mkdir -p squashfs-root/usr/share/applications && cp ./squashfs-root/sudachi.desktop ./squashfs-root/usr/share/applications
mkdir -p squashfs-root/usr/share/icons && cp ./squashfs-root/sudachi.svg ./squashfs-root/usr/share/icons
mkdir -p squashfs-root/usr/share/icons/hicolor/scalable/apps && cp ./squashfs-root/sudachi.svg ./squashfs-root/usr/share/icons/hicolor/scalable/apps
mkdir -p squashfs-root/usr/share/pixmaps && cp ./squashfs-root/sudachi.svg ./squashfs-root/usr/share/pixmaps
...
version=$(cat /sudachi/README.md | grep -o 'early-access [[:digit:]]*' | cut -c 14-17) 
...
mkdir -p /sudachi/artifacts/version
# Version AppImage
...
# Build AppDir
/tmp/squashfs-root/AppRun $HOME/squashfs-root/usr/bin/sudachi -unsupported-allow-new-glibc -no-copy-copyright-files -no-translations -bundle-non-qt-libs
...
mv ./sudachi-pineapple-x86_64.AppImage /sudachi/artifacts/version/Sudachi-EA-$version.AppImage
...
/tmp/squashfs-root/usr/bin/appimagetool $HOME/squashfs-root -u "gh-releases-zsync|pineappleEA|pineapple-src|continuous|sudachi-x86_64.AppImage.zsync"
#	mv sudachi-x86_64.AppImage old.AppImage
#VERSION=pineapple /tmp/mkappimage.AppImage --appimage-extract-and-run $HOME/squashfs-root -u "gh-releases-zsync|pineappleEA|pineapple-src|continuous|sudachi-x86_64.AppImage.zsync"

#mv sudachi-pineapple-x86_64.AppImage sudachi-x86_64.AppImage
mv sudachi-x86_64.AppImage* /sudachi/artifacts

cp -R $HOME/artifacts/ /sudachi/
#cp "$BUILDBIN"/sudachi /sudachi/artifacts/version/
chmod -R 777 /sudachi/artifacts
cd /sudachi/artifacts
ls -al /sudachi/artifacts/
ls -al /sudachi/artifacts/version
