...
ver=$(cat /sudachi/README.md | grep -o 'early-access [[:digit:]]*' | cut -c 14-17)
title="sudachi Early Access $ver"

sudachipatch=( $(ls -d patches/* ) )
for i in "${sudachipatch[@]}"; do patch -p1 < "$i"; done
